...
fact2(I,K,N,X):-I1 is I+1,K1 is K*I1,fact2(I1,K1,N,X).
        ...
        % Число Фибоначи заданного номера 
fibo(1,1):-!.
fibo(2,1):-!.
...
fibo2(J,K,I,N,X):-I1 is I+1, K1 is J+K, fibo2(K,K1,I1,N,X).
        ...
        % Максимум из двух и трёх
max(X,Y,X):-X>Y,!.
max(_,Y,Y).

max3(X,Y,U,Z):-max(Y,U,K),max(X,K,Z).

        % Простота и Наибольший простой делитель
pr(2):-!.
pr(X):-pr1(2,X).
pr1(X,X):-!.
...
npd(I,N,X):-I1 is I-1,ndp(I1,N,X).
        ...
        %сумма цифр числа
sum(X,X):- X<10, !.
sum(X,S):-L is X mod 10, B is X div 10, sum(B,S1), S is S1+L.
...
pr3_1:-read(N), r_list(A,N), sum_list(A,Z), write(Z).
%Читаем число, вводим список длины N, sumlist, вывод ответа.
